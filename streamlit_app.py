    ...
    with col2:
        with st.container(border=True):
            ...
            <div style="text-align: center; margin-bottom: 20px; padding-top: 10px;">
                ...
                <h3 style="color: var(--text-color); font-family: 'Plus Jakarta Sans', sans-serif; font-size: 1.3rem; font-weight: 700; margin: 0;">Sign in with Google</h3>
            ...
            if not client_id or not client_secret:
                st.warning(" Google OAuth credentials are not fully configured. Set them up below to enable actual Google Sign-In.")
...
if page == "Home":
    ...
    st.markdown("""
    <h2 style="margin-top: 0;"> Empowering Executive Decisions with Predictability</h2>
    ...
    st.markdown("""
    <h3 style="margin-top: 20px;"> Key Value Proposition:</h3>
    ...
    if not st.session_state.logged_in:
        st.markdown("<p style='color: #6366F1; font-weight: 600; margin-top: 20px;'> Authentication required. Please sign in via the Navigation sidebar.</p>", unsafe_allow_html=True)
...
# =========================================================
    ...
    st.markdown("<h2 style='text-align: center; margin-top:0;'> Security Gateway</h2>", unsafe_allow_html=True)
...
# =========================================================
    ...
    with col2:
        ...
        if st.session_state.df_details is None:
            st.info(" Please upload a dataset or load the sample dataset to map columns.")
        else:
    ...
    if st.session_state.df_details is not None:
        ...
        if not is_backend_alive:
            st.subheader(" Clean Dataset in Memory")
...
# =========================================================
    ...
    if st.session_state.df_details is None:
        st.warning(" Upload a SaaS dataset or load our template under 'Upload / Manage Dataset' before viewing analytics.")
    else:
        try:
            ...
            with col_chart3:
                st.subheader(" Support Load vs. Churn Rate Correlation")
            ...
            with col_chart4:
                ...
                if kpis["ltv_cac_ratio"] < 3.0:
                    insights.append(" **Critical Unit Economics:** Your LTV to CAC ratio is currently below the industry benchmark of 3.0x. Consider reducing marketing spends or increasing monthly prices.")
                else:
...
# =========================================================
    ...
    if st.session_state.df_details is None:
        st.warning(" Upload a SaaS dataset or load our template under 'Upload / Manage Dataset' before viewing cohort analysis.")
    else:
        try:
            m = st.session_state.mapped_columns
...
# =========================================================
    ...
    if st.session_state.df_details is None:
        st.warning(" Upload a SaaS dataset or load our template under 'Upload / Manage Dataset' before training the predictor.")
    else:
